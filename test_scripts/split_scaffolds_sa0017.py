...
filename = '349.g.vcf.gz'
print(filename)

with gzip.open(os.path.join(A_vcf_dir, filename), "rb") as file_in:
    with gzip.open(os.path.join(A_vcf_dir, "sa0017", filename), "wb") as file_out:

        for line in file_in:
            if b'sa0017' in line or b'#' in line:
                    file_out.write(line)
            if b'sa0018' in line and b'#' not in line:
                break
